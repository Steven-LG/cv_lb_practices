...
# new_red = cv2.bitwise_and(red_image.astype(np.uint8), red_image.astype(np.uint8), mask=filled_mask.astype(np.uint8))

# new_image = cv2.bitwise_and(image.astype(np.uint8), image.astype(np.uint8), mask=filled_mask.astype(np.uint8))
# final_image = np.where(binary_mask[:, :, np.newaxis] == 255, new_red, new_image)
